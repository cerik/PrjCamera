...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(TaskCmdHandleNa, TaskCmdHandle, osPriorityNormal, 0, 128);
  TaskCmdHandleNaHandle = osThreadCreate(osThread(TaskCmdHandleNa), NULL);

  /* definition and creation of TaskDBGatherNam */
  osThreadDef(TaskDBGatherNam, TaskDBGather, osPriorityIdle, 0, 128);
...
void TaskCmdHandle(void const * argument)
{
  ...
  /* USER CODE BEGIN TaskCmdHandle */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END TaskCmdHandle */
}
