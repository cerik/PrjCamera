...
/**函数说明： 
 * 执行CRC生成的C语言的函数在下面标出，所有可能的CRC值都被预装在两个数组当中，
 * 当计算报文内容时可以简单的索引即可； 
 * 一个数组包含有16位CRC域的所有256个可能的高位字节，宁一个数组含有低位字节的
 * 值；这种索引访问CRC的方式提供了比对报文缓冲去的每一个新字符都计算新的CRC更
 * 快的方法；注意：此程序内部执行高/低CRC字节的交换。此函数返回的是已经经过交
 * 换的CRC值；也就是说，该函数的返回值可以直接放置于报文*用于发送； 
 */
...
UINT16 UpdateCrc16(UINT8 ch,UINT16 crc16)
{
    ...
    index = crcH ^ ch;
    crcH  = crcL ^ auchCRCHi[index];
    crcL  = auchCRCLo[index];
    return  crcH<<8 | crcL;
}
